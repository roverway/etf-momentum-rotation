...
```
涨幅 = (最新收盘价 − 22天前收盘价) ÷ 22天前收盘价
```

数据来源为 AKShare `fund_etf_hist_sina` 接口返回的日线数据中的 `close` 列。
回看期固定为 `CHECK_RANGE = 22` 个交易日。
每日对ETF池中所有标的计算涨幅，选取涨幅 > 0 且涨幅最大的ETF持有；若全部下跌则空仓。
...
每日循环流程：
1. 进入交易日 → 2. 用截至当日的数据计算动量信号 → 3. 若需换仓则取当日收盘价执行买卖 → 4. 记录成交
